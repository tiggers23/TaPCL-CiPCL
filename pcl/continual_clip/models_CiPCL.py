...
from .utils import get_class_ids_per_task, random_mask
from . import utils
...
class ClassIncremental(nn.Module):
    ...
    def train(self, task_id, cfg, train_dataset, train_classes_names):
        ...
        for iteration in loop:
            scheduler(iteration)
            try:
                inputs, targets, texts, imgid = next(train_iter)
            except:
                train_iter = iter(train_loader)
                inputs, targets, texts, imgid = next(train_iter)

            texts = list(texts)
            texts = random_mask(texts, cfg.mask)
            inputs, targets = inputs.to(self.device), targets.to(self.device)     
            logits = self.model(inputs, texts, 0, is_train=True)  # 分开
            loss = (self.nceloss(logits)+self.nceloss(logits.t()))/2*0.5
