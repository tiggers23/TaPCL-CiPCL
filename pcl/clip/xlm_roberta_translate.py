...
# xlm_roberta-sharing-token-sim-alt
...
class XLMRobertaLayer(nn.Module):
    def __init__(self, config):
        ...
        self.ffn_num = 256
        self.adaptmlp = Adapter(d_model=768, dropout=0.1, bottleneck=self.ffn_num,
                                    init_option='lora',
                                    adapter_scalar=0.1,
    ...
    def forward(
        ...
        attentions = outputs[0] if not global_zeroshot and global_is_train and global_get_attentions and layerid == 11 else None

        adapter_output = self.adaptmlp(attention_output, add_residual=False)
        ...
        if global_zeroshot or layerid !=11:
            # if layerid == 0 and global_get_attentions and global_is_train:
            #     outputs = (layer_output,) + outputs + (attentions,)
            # else:
                outputs = (layer_output,) + outputs
        # elif global_target_lang and global_is_train:
        #     outputs = (layer_output + adapter_output,) + outputs
        else:
            if global_get_attentions:
                outputs = (layer_output,) + outputs + (attentions,) 
            else:               
                outputs = (layer_output + adapter_output,) + outputs   
...
class XLMRobertaEncoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            output_attentions = True if i == 11 and not global_zeroshot and global_is_train and global_get_attentions else False
            ...
            if global_get_attentions and global_is_train and not global_zeroshot:
                if i == 11:
...
class XLMRobertaModel(XLMRobertaPreTrainedModel):
    ...
    def forward(
        ...
        use_cache (`bool`, *optional*):
            ...
            `past_key_values`).
        """
        global global_is_train, global_zeroshot, global_get_attentions
        global_is_train = is_train
        global_zeroshot = zeroshot
        global_get_attentions = get_attentions
